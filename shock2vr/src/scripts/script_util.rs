...
pub fn play_impact_sound(
    ...
    // A world surface names itself; an entity's own material is what was hit,
    // and resolves exactly as it always has.
    let material = surface_material
        .map(str::to_owned)
        .unwrap_or_else(|| get_impact_material(world, hit_entity_id));
    // A world surface is newly able to name itself, so each query keeps the
    // default it replaced as a fallback: the schema has no bullet on glass.
    let with_default_fallback = |build: &dyn Fn(&str) -> EnvSoundQuery| {
        let query = build(&material);
        match surface_material {
            Some(surface) if surface != DEFAULT_IMPACT_MATERIAL => {
                query.with_fallback(build(DEFAULT_IMPACT_MATERIAL))
            }
            _ => query,
        }
    };
    let query = with_default_fallback(&|material| {
        get_environmental_sound_query(world, entity_id, "collision", vec![("material", material)])
            .unwrap_or_else(|| {
                ...
                let own_material = get_impact_material(world, entity_id);
                let (first, second) = if own_material.as_str() < material {
                    (own_material.as_str(), material)
                } else {
                    (material, own_material.as_str())
                };
    ...
    let fallback = world
        ...
        .then(|| {
            with_default_fallback(&|material| {
                dark::EnvSoundQuery::from_tag_values(vec![
                    ("event", "collision"),
                    ("weapontype", "wrench"),
                    ("material", material),
