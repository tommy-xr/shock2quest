...
pub fn render_pathfinding_debug(path_database: &PathDatabase) -> Vec<SceneObject> {
    ...
    for link in &path_database.links {
        ...
        if from_cell_idx < path_database.cells.len() && to_cell_idx < path_database.cells.len() {
            let from_center = path_database.cells[from_cell_idx].center + Vector3::new(0.0, PATH_NODE_HEIGHT, 0.0);
            let to_center = path_database.cells[to_cell_idx].center + Vector3::new(0.0, PATH_NODE_HEIGHT, 0.0);
