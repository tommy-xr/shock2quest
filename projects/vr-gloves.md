...
- `assets/vr_glove_emissive.png` — grayscale light mask in the same UV atlas
  (cuff band, fingertip pads, back-of-hand stripes), 1024 like the colour map.
  **Hand-authored**; `tools/make_vr_glove_emissive.py` only painted the first
  draft, so don't rerun it over an edited file.
...
- **A light on the glove.** `assets/vr_glove_emissive.png` is a grayscale mask
  ...
  material's normal shading. The PNG is **hand-authored** at the colour map's
  1024; `tools/make_vr_glove_emissive.py` only painted its first draft and must
  not be rerun over an edited file.

  That draft is worth describing because it is what ships today. The front
  comes from the *model*: a band round the wrist cuff plus a pad on each
  fingertip, picked in model space (depth along the fingers, and a radius around
  the five `finger_*_aux` markers) and rasterized through the mesh's own UVs, so
  it lands correctly wherever the atlas packs a part. The back comes from the
  *colour map*: the light stripes down the back-of-hand panel are an albedo
  detail, not a shape, so model space cannot separate them from the panel around
  them - they are picked as texels standing well above a dark neighbourhood
  inside the back-of-hand box, then opened to drop the panel's perforations.
  ...
  Lighting the front *and* the back is what makes the state readable from either
  side; cuff-and-fingertips alone vanishes from behind the hand, which is the
  side the player looks at most. Row 3 of `debug_gloves` shows all four states
  side by side.

  The mask is 1024 to stay paintable, and the texture loader expands it to RGBA
  on upload - 4 MiB of Quest VRAM for content with no detail above a few texels.
  Downscaling it *on load* (not in the asset) is the obvious saving if that ever
  matters.
