...
- AI lacks alertness progression, goal arbitration, and path planning; behaviours jump straight from idle to chase without the intermediate states provided in `src/ai/aialert.h` and `src/ai/aimove.cpp` in the original engine.
- Security devices (cameras, turrets) do not implement the green/yellow/red FSM from TODO.md nor broadcast alarms through SwitchLink/SecuritySystem.

## Animation Pipeline Deep Dive
...
- `AnimationEvent::VelocityChanged` should prompt an immediate velocity update to keep footsteps in sync. Currently the event is ignored, so entities move at the previous clip’s speed for one frame after each transition.
- Direction changes rely on `MotionStuff::end_direction`, but halving the angle (`ang * -0.5`) under-rotates the actor, which the steering code compensates for with abrupt yaw adjustments.
...
4. Implement an alertness manager and pathfinding layer so behaviours escalate naturally and can navigate around obstacles.
5. Rebuild security-device logic: cameras follow Idle → Warning → Alarm with timers, propagate switches, and turrets respect global alert state before arming.
6. Extend property/link parsing to cover the data required by alertness, security, and navigation systems.
...
### Phase 3 – Alertness and Pathfinding
- **Prop ingestion:** Parse `AI_AlertCap`, `AI_AlertResponse`, `AI_AlertnessSet`, and sensing-tuning props (`AI_Listen`, `AI_Vision`) so designers can configure awareness thresholds. Surface `AIWatchObj` data we already read through the new alertness manager.
- Add an `AlertState` component and manager that tracks awareness levels (sleep → low → moderate → high) using LOS checks (`is_player_visible`) and sound triggers.
- Update behaviours to consult alertness before escalating (idle → search → chase), ensuring scripted sequences can raise or lower alert levels.
- Build a nav-graph from mission cell data and implement A* pathfinding. Expose path-following steering that feeds into existing collision avoidance.
- **Link ingestion:** Import mission `PathCell`/`PathEdge` data into the nav graph, and bring in `AIPath`/`AIGoalLoc` links when present to respect designer-authored waypoints or patrols.
- Acceptance: AI idles until hearing/seeing the player, transitions through search, and can navigate around obstacles instead of beelining.

### Phase 4 – Security Device Ecology
- **Prop/Link ingestion:** Parse `AI_Device`/`AI_Camera` parameters (scan angles, timers) and `SwitchLink` / `SecurityCamera` relations so cameras can broadcast alarms. Capture any global security quest bits (`AI_Security`, `SecurityState`) required to coordinate turrets.
- Implement the camera FSM from TODO.md: Idle (green), Tracking (yellow with timer), Alarm (red) that emits SwitchLink actions and raises alertness.
...
### Validation & Tooling
- Add debug overlays/logs for animation blending (current clip, next clip, blend progress) and alertness levels.
- Create targeted regression scenarios for scripted sequences, ranged enemies, and security devices to verify the fixes.
- Document new systems in developer notes and update TODO.md items once complete.
...
- Add deserializers for alertness-related properties (`AI_AlertCap`, `AI_AlertResponse`, `AI_AlertnessSet`, `AI_Listen`, `AI_Vision`) so alertness logic can respect designer tuning.
...
- Expand link/property support for security devices:
  - `AI_Device`, `AI_Camera`, and related parameter blocks.
