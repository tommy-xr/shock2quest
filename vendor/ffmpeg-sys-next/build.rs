...
fn search_include(include_paths: &[PathBuf], header: &str) -> String {
    ...
    // For Android cross-compilation, check FFMPEG_INCLUDE_DIR environment variable
    if env::var("TARGET").unwrap_or_default().contains("android") {
        if let Ok(ffmpeg_include) = env::var("FFMPEG_INCLUDE_DIR") {
            let android_include = Path::new(&ffmpeg_include).join(header);
            if fs::metadata(&android_include).is_ok() {
                return android_include.to_str().unwrap().to_string();
            }
        }
    }
