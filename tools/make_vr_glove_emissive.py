...
- Front: a small pad on each fingertip, picked in
...
TIP_SOLID, TIP_FADE = 0.025, 0.045
...
def vertex_weights(positions, tips, span):
    """Small fingertip accents. Keep the cuff/interior textile unlit."""
    weight = np.zeros(len(positions), dtype=np.float32)
    for tip in tips:
        distance = np.linalg.norm(positions - tip, axis=1) / span
        weight = np.maximum(weight, 0.5 * smooth_falloff(distance, TIP_SOLID, TIP_FADE))
    return weight
...
def back_of_hand_stripes():
    ...
    A hand seen from behind shows only the far edge of the
