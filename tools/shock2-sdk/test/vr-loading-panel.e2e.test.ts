...
/** Matches shock2vr::ui::FRONTEND_PANEL_DISTANCE. */
...
const distanceFromOrigin = ([x, y, z]: Vec3): number =>
  Math.hypot(x, y, z);
...
test(
  ...
  async () => {
    ...
    // Every one of them is out on the panel, not pasted at the origin. The
    // panel hangs a frontend distance along the gaze at eye height; the
    // per-layer z-step spreads them by a millimetre, hence the tolerance.
    for (const position of positions) {
      assert.ok(
        Math.abs(distanceFromOrigin(position) - Math.hypot(PANEL_DISTANCE, PLAYER_EYE_HEIGHT_WORLD)) < 0.05,
        `a VR loading-screen element should sit on the world panel, drew at ${JSON.stringify(position)}`,
      );
      assert.ok(
