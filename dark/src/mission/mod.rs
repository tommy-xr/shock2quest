...
impl SystemShock2Geometry {
    /// A water boundary (texture 247/248): rendered, but never solid.
    pub fn is_water_surface(&self) -> bool {
        matches!(self.texture_idx, 247 | 248)
    }
}
...
impl WaterRenderInfo {
    /// The WATERHW texture for a water boundary poly: 247 is the "in"
    /// side, 248 the "out" side, prefixed by the cell's flow group.
    pub fn texture_name(&self, texture_num: u16, flow_group: u8) -> String {
        let prefix = self
            .texture_prefixes
            .get(flow_group as usize)
            .unwrap_or(&self.default_prefix);
        let suffix = if texture_num == 247 { "in" } else { "out" };
...
fn read_water_render_info<T: io::Read + io::Seek>(
    ...
    let texture_prefixes = read_chunk_bytes(table_of_contents, reader, "FLOW_TEX")
        ...
        .unwrap_or_else(|| vec![default_prefix.clone(); 256]);
